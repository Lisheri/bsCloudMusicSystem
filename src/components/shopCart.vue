      ...
      <div class="container">
        ...
        <div class="section2">
          <div class="clear">
            ...
            <div class="fl">
              <div class="productInfo listitem">商品信息</div>
            </div>
            <div class="fl">
              <div class="kong listitem"></div>
            </div>
            <div class="fl">
              <div class="unitPrice listitem">单价</div>
            </div>
            <div class="fl">
              <div class="number listitem">数量</div>
            </div>
            <div class="fl">
              <div class="amountOfMoney listitem">金额</div>
            </div>
            <div class="fl">
              <div class="operation listitem">操作</div>
            </div>
          </div>
        </div>
        <div class="section3">
          <div class="cartBox">
            ...
            <ul class="cartBoxUl">
              <li class="cartBoxLi clear" v-for="(item, index) in cartList" :key="index">
                <el-checkbox
                  ...
                  @change="handlecheck(item.goodsNum, item.id, checkedArr[index])"
                ></el-checkbox>
                ...
                <div class="fl num">
                  <el-input-number
                    ...
                    @change="handleChange(item.goodsNum, item.id, checkedArr[index])"
                    :min="1"
                    :max="item.stock"
                    label="描述文字"
                    class="numInput"
                ...
                <div
                  class="fl allPrice"
                  :title="item.singlePrice * item.goodsNum"
                >¥{{item.singlePrice * item.goodsNum}}</div>
                <div class="fl operation" @click="handleDelete(item.id)">
            ...
            <div class="boxBottom">
              <el-row>
                ...
                <el-col :span="22">
                  <el-button @click="buyNow">去结算</el-button>
                </el-col>
              </el-row>
            </div>
          </div>
  ...
  methods: {
    ...
    async buyNow() {
      ...
      this.cartList.forEach(item => {
        if (item.checked) {
          let { goodsNum, id, imgUrl, name, singlePrice, type } = item;
          params.push(
            Object.assign(
              {},
              {
                goodsInfo: { img: imgUrl, type, name },
                singlePrice,
                goodsNum,
                allPrice: singlePrice * goodsNum
              }
            )
          );
        }
      ...
      this.$router.push({
        path: "/goods/order",
        query: { arr: JSON.stringify(params) }
      });
    }
  },
  components: { Head },
